...
var instructor=new staffMembers ({
...
var newCourse = new course(
    {
        ...
        coordinator: TA2._id, // id of the Coordinator of this course
...
await staffMembers.findOneAndUpdate({_id :
    instructor._id},  { $push: {  courses: newCourse.courseName}}, {new: true});
await staffMembers.findOneAndUpdate({_id :
    TA._id},  { $push: {  courses: newCourse.courseName}}, {new: true});
await staffMembers.findOneAndUpdate({_id :
    TA2._id},  { $push: {  courses: newCourse.courseName}}, {new: true});
...
await course.findOneAndUpdate({_id :
    ...
    await staffMembers.findOneAndUpdate({_id :
            HOD._id},  { $push: {  courses: newCourse.courseName}}, {new: true});
     loc = new location(
        {
            roomNr: 'H14',
            roomType: 'lecture hall', //only posible values are lecture halls, tutorial rooms, labs and offices
   ...
   newSlot.save();
    ...
    await staffMembers.findOneAndUpdate({_id :
            Coordinator._id},  { $push: {  courses: newCourse.courseName}}, {new: true}); 
     await staffMembers.findOneAndUpdate({_id :
            ta._id},  { $push: {  courses: newCourse.courseName}}, {new: true}); 
