...
function editpage(){
    ...
    const [options2,setOptions2]= useState([]);// Load academic Dropdown
    const [alertext,setAlertext]= useState();
    const [value1,setValue1]= useState(''); // This is the course
    const [value2,setValue2]= useState(''); // This is the SlotID
    useEffect(async ()=>{
    ...
    const assignInstrReq= ()=> {
        if(value1&&value2){
        axios.get('http://localhost:5000/api/instructor/assign-course/course',{slotID:value1,academicID: value2},{headers:{'auth-token':token}}).then((res)=>{
