...
test('Faculty add test', async ()=>{
    await FacultyModel.deleteMany({})
    department = await DepartmentModel.find({"departmentName":"MET"})
...
//     course = await CourseModel.findOne({"courseName":"CSEN 701 - Embedded Systems" })
//     staffMem1 = StaffModel.findOne({"name":"Instructor 1"})
//     department = await DepartmentModel.find({"departmentName":"MET"})
...
//     course = CourseModel.findOne({"courseName": "CSEN 701 - Embedded Systems"})
//     staffMem1 = StaffModel.findOne({"name":"Instructor 1"})
//     staffMem1 = StaffModel.findOne({"name":"Instructor 2"})
//     replaceSlot = SlotModel.findOne({"courseTaughtInSlot" : course._id})
